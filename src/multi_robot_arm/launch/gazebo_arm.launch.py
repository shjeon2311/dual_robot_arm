...
def generate_launch_description():
    ...
    arm_params = load_yaml(
        package_path, "config/ur/" + robot_type + "/dual_robot.yaml"
    )

    robot_final_action = None
    # arm1 로봇 생성
    robot_final_action = spawn_robot(ld, "ur5", arm_params["arm1"]["name"], 
                use_sim_time, arm_params["arm1"]["x_pose"], 
                arm_params["arm1"]["y_pose"], arm_params["arm1"]["Y"], 
                robot_final_action,)

    # arm2 로봇 생성    
    robot_final_action = spawn_robot(ld, "ur5", arm_params["arm2"]["name"], 
                use_sim_time, arm_params["arm2"]["x_pose"], 
                arm_params["arm2"]["y_pose"], arm_params["arm2"]["Y"], 
                robot_final_action,)
...
def spawn_robot(
    ...
    message = """ {
            ...
            'points': [
                {
                'positions': [1.5708, 1.5708, 2.35619, 1.5708, 0.349066, 1.22173],
                'velocities': [],
                'accelerations': [],
                'effort': [],
                'time_from_start': {
